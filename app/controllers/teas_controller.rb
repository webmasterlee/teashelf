class TeasController < ApplicationController
  ...
  def show
    ...
    @otherNotes = Tea.where("name = ? and user_id != ? and notes is not null and notes !=''", @tea.name, current_user.id).limit(10)
    ...
    @suggestions = Tea.find_by_sql(["select distinct on (name) name,vendor,url from teas where tea_type_id = ? and upper(name) != ? and user_id in (select user_id from teas where upper(name) = ? and user_id != ?) and upper(name) not in (select upper(name) from wishlists where user_id = ?) and upper(name) not in (select upper(value) from exclusions where user_id = ? and attr = 'tea_name') limit 10", @tea.tea_type_id, @tea.name.upcase,@tea.name.upcase, current_user.id, current_user.id, current_user.id ])
  end
